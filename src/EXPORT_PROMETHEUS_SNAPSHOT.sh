...
kubectl --kubeconfig $KUBE_CONFIG_PATH exec -it -n monitoring $pmt_pod_name -- rm -r /prometheus/snapshots

echo "sending request for generating prometheus snapshot on pod: $pmt_pod_name"
pmt_pod_snapshot_gen_result=`curl -XPOST http://localhost:9090/api/v1/admin/tsdb/snapshot`
...
if [[ $pmt_pod_snapshot_gen_result == *"success"* ]]; then
    ...
    kubectl --kubeconfig $KUBE_CONFIG_PATH cp -n monitoring $pmt_pod_name:/prometheus/snapshots/ $snapshot_target_dir
else
